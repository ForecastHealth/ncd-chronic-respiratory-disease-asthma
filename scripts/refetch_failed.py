#!/usr/bin/env python3
"""
Re-fetch failed analytics data by comparing validation_results.csv with successfully fetched files.

This script identifies entries that failed during the initial fetch and re-fetches them.

Usage:
    python3 refetch_failed.py --csv validation_results.csv
    python3 refetch_failed.py --csv validation_results.csv --analytics-types echo balance_set
"""
...
def find_failed_entries(csv_path, output_dir, analytics_types):
    """Find entries that failed by comparing CSV with existing files."""
    ulid_data = load_ulids_from_csv(csv_path)
    
    # Map analytics types to file suffixes
    type_mapping = {
        'EDGE_VALUES_CALCULATED': 'edge',
        'BALANCE_SET': 'balance',
        'ECHO': 'echo'
    }
    ...
    for item in ulid_data:
        ulid = item['ulid']
        scenario = item['scenario']
        
        for event_type in analytics_types:
            suffix = type_mapping[event_type]
            
            # Look for existing files with this ULID, scenario, and event type
            pattern = f"{ulid}_{scenario}_{suffix}_*.json"
            matching_files = glob.glob(os.path.join(output_dir, pattern))
            
            if not matching_files:
                # No file found - this is a failed entry
...
async def process_request(session, semaphore, failed_entry, output_dir, request_num, total_requests):
    """Process a single request with semaphore for concurrency control."""
    async with semaphore:
        ulid = failed_entry['ulid']
        scenario = failed_entry['scenario']
        country = failed_entry['country']
        ...
        print(f"[{request_num}/{total_requests}] Re-fetching {ulid} ({country}, {scenario}, {event_name})... ", end='', flush=True)
...
async def main_async(args):
    ...
    # Find failed entries
    print(f"Analyzing {args.csv} to find failed entries...")
    failed_entries = find_failed_entries(args.csv, args.output_dir, event_types)
    
    if not failed_entries:
        print("No failed entries found! All ULIDs appear to have been successfully fetched.")
        return 0
    
    total_requests = len(failed_entries)
    print(f"Found {total_requests} failed entries to re-fetch...")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        for i, failed_entry in enumerate(failed_entries, 1):
            task = process_request(session, semaphore, failed_entry, output_dir, i, total_requests)
            tasks.append(task)
        ...
        # Summary
        print("-" * 60)
        print(f"Re-fetch complete: {successful} successful, {failed} still failed")
...
def main():
    parser = argparse.ArgumentParser(description='Re-fetch failed analytics data from localhost:8000')
    parser.add_argument('--csv', required=True,
                       help='CSV file containing ULIDs (e.g., validation_results.csv)')
